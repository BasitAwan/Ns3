...
EpcX2::RecvFromX2cSocket (Ptr<Socket> socket)
{
  ...
  if (procedureCode == EpcX2Header::ReplicateState )
  {
    ...
    m_x2SapUser->RecvStateReplication (params);

  }
  else if (procedureCode == EpcX2Header::HandoverPreparation)
    {
...
void
EpcX2::DoSendStateReplicate(EpcX2SapProvider::HandoverRequestParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  EpcX2Header x2Header;
  x2Header.SetProcedureCode (EpcX2Header::ReplicateState);
  x2Header.SetLengthOfIes (x2HoReqHeader.GetLengthOfIes ());
  x2Header.SetNumberOfIes (x2HoReqHeader.GetNumberOfIes ());
