    ...
    case CONNECTED_NORMALLY:
      {
        ...
        m_rrc->m_x2SapProvider->SendStateReplicate (params);
        SwitchToState (HANDOVER_PREPARATION);
      }
      break;
...
void
LteEnbRrc::SendStateReplicate (uint16_t rnti, uint16_t cellId)
{
  NS_LOG_FUNCTION (this << rnti << cellId);
  NS_LOG_LOGIC ("Request to send HANDOVER REQUEST");
  NS_ASSERT (m_configured);
  std::cout << Simulator::Now ().GetSeconds ()<< " SendStateReplicate" << std::endl;
  Ptr<UeManager> ueManager = GetUeManager (rnti);
...
//State Replication
void
LteEnbRrc::DoRecvStateReplication (EpcX2SapUser::HandoverRequestParams req)
{
...
LteEnbRrc::DoRecvUeContextRelease (EpcX2SapUser::UeContextReleaseParams params)
{
  ...
  uint16_t rnti = params.oldEnbUeX2apId;
  std::cout << Simulator::Now ().GetSeconds ()<< " DoRecvUEContextRelease" << std::endl;
...
LteEnbRrc::DoTriggerHandover (uint16_t rnti, uint16_t targetCellId)
{
  ...
  if (isHandoverAllowed)
    ...
    std::cout << Simulator::Now ().GetSeconds ()<< " DoTriggerHandOver" << std::endl;
